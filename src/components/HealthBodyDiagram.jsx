...
import { Activity, Brain, Heart, Wind, Droplets, Info, Thermometer, User, ShieldCheck, AlertCircle, Sparkles } from 'lucide-react';

const BodyPart = ({ id, path, label, onClick, color, isCore = true, isWarning = false }) => (
    <motion.path
        id={id}
        d={path}
        fill={color || "rgba(148, 163, 184, 0.1)"}
        stroke={isWarning ? "#ef4444" : "rgba(255, 255, 255, 0.2)"}
        strokeWidth={isCore ? (isWarning ? 1.5 : 0.5) : 1}
        initial={isWarning ? { opacity: 0.6 } : {}}
        animate={isWarning ? {
            opacity: [0.6, 1, 0.6],
            fill: [color, "rgba(239, 68, 68, 0.6)", color]
        } : {}}
        ...
        style={{ cursor: 'pointer', originX: '100px', originY: '150px' }}
    />
);
...
const HealthBodyDiagram = ({ metrics }) => {
    ...
    return (
        <div className="glass-panel" style={{ padding: '2rem', height: '100%', minHeight: '600px', display: 'flex', flexDirection: 'column', position: 'relative', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', zIndex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    {metrics && (
                        ...
                        >
                            <Sparkles size={12} /> Linked to AI Analysis
                        </motion.div>
                    )}
            ...
            <div style={{ minHeight: '350px', position: 'relative', display: 'flex', justifyContent: 'center', perspective: '1000px' }}>
                <svg viewBox="0 0 200 300" style={{ height: '350px', maxWidth: '350px', filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.3))' }}>
                    ...
                    <BodyPart id="leftArm" path={bodyPaths.leftArm} label="Left Hand" onClick={() => setSelectedPart(parts[5])} color={parts[5]?.color} isCore={false} isWarning={parts[5]?.score < 50 && parts[5]?.score !== null} />
                    <BodyPart id="rightArm" path={bodyPaths.rightArm} label="Right Hand" onClick={() => setSelectedPart(parts[5])} color={parts[5]?.color} isCore={false} isWarning={parts[5]?.score < 50 && parts[5]?.score !== null} />
                    <BodyPart id="leftLeg" path={bodyPaths.leftLeg} label="Left Leg" onClick={() => setSelectedPart(parts[5])} color={parts[5]?.color} isCore={false} isWarning={parts[5]?.score < 50 && parts[5]?.score !== null} />
                    <BodyPart id="rightLeg" path={bodyPaths.rightLeg} label="Right Leg" onClick={() => setSelectedPart(parts[5])} color={parts[5]?.color} isCore={false} isWarning={parts[5]?.score < 50 && parts[5]?.score !== null} />

                    <BodyPart id="head" path={bodyPaths.head} label="Brain" onClick={() => setSelectedPart(parts[0])} color={parts[0]?.color} isWarning={parts[0]?.score < 50 && parts[0]?.score !== null} />
                    <BodyPart id="heart" path={bodyPaths.heart} label="Heart" onClick={() => setSelectedPart(parts[1])} color={parts[1]?.color} isWarning={parts[1]?.score < 50 && parts[1]?.score !== null} />
                    <BodyPart id="liver" path={bodyPaths.liver} label="Liver" onClick={() => setSelectedPart(parts[2])} color={parts[2]?.color} isWarning={parts[2]?.score < 50 && parts[2]?.score !== null} />
                    <BodyPart id="stomach" path={bodyPaths.stomach} label="Stomach" onClick={() => setSelectedPart(parts[4])} color={parts[4]?.color} isWarning={parts[4]?.score < 50 && parts[4]?.score !== null} />
                    <BodyPart id="kidneys" path={bodyPaths.kidneys} label="Kidneys" onClick={() => setSelectedPart(parts[3])} color={parts[3]?.color} isWarning={parts[3]?.score < 50 && parts[3]?.score !== null} />
                </svg>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                                <div style={{ background: 'var(--color-primary-soft)', padding: '0.4rem', borderRadius: '8px' }}>
                                    {(() => {
                                        ...
                                        return PartIcon ? <PartIcon size={18} className="text-primary" /> : null;
                                    })()}
                                </div>
                    ...
                    {parts.map(part => (
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                                {(() => {
                                    const CardIcon = part.icon;
                                    return CardIcon ? <CardIcon size={16} color={part.score === null ? '#64748b' : part.score > 80 ? '#22c55e' : '#ef4444'} /> : null;
                                })()}
